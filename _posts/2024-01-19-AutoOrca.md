...
```bash
AutoOrca.sh structureFile [--template(-t) <模板文件>] [--suffix(-s) <输入文件后缀名>] [--nprocs(-n) <核心数>] [--memory(-m) <内存>] 
```
...
- 示例2：生成`water.fchk`中水分子的ORCA输入文件，使用`tmpl1.inp`、`tmpl2.inp`和`tmpl3.inp`作为模板，生成的输入文件名分别为`water_task1.inp`、`water_task2.inp`和`water_task3.inp`

```bash
AutoOrca.sh water.fchk --template tmpl1.inp tmpl2.inp tmpl3.inp --suffix task1 task2 task3
```

- 示例3：生成`water.xyz`中水分子的ORCA输入文件，使用`tmpl1.inp`和`tmpl2.inp`作为模板，生成的输入文件名分别为`water_task1.inp`和`water_task2.inp`，每个计算均使用12个核心，每核心内存限制为2000MB
