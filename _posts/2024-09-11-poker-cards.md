...
在无穷牌堆中，随机抽取一张扑克牌其为指定花色的概率恒为 $\displaystyle{\frac14}$。
...
<img src="/assets/images/poker card infinite.svg" width="400"/>
...
$$\sum_{k=0}^np^k\binom nk=(p+1)^n$$

证明：

$$\begin{align}\sum_{k=0}^np^k\binom nk&=\sum_{k=0}^n\binom nk\cdot p^k\cdot1^{n-k}=(p+1)^n\end{align}$$
